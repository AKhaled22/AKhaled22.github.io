...
const IICMaintenance = () => {
  ...
  const handleSubmit = (values, errors) => {
    ...
    if (currCBG < 80) {
      setResultsElement(
        "DC insulin and give 100 mL D10, recheck every 15 minutes, give 100 mL D10 again if still < 100. When CBG > 140 mg%, " +
          (currRate * 0.5 < 0.5
            ? "inform responsible doctor"
            : "resume insulin by " + Math.round(currRate) * 0.5 + " mL/hr")
      );
    } else if (currCBG < 100) {
      setResultsElement(
        "DC insulin and give 100 mL D10, recheck every 30 minutes, give 100 mL D10 again if still < 100. When CBG > 140 mg%, " +
          (currRate * 0.5 < 0.5
            ? "inform responsible doctor"
            : "resume insulin by " + Math.round(currRate) * 0.5 + " mL/hr")
      );
      ...
      if (differenceCBG > 0) {
        ...
        setResultsElement(
          y < 0.5
            ? "Inform responsible doctor"
            : "Decrease rate to " + y + " ml/hr"
        );
      } else {
        setResultsElement(
          "DC insulin -> CBG/30 min -> when CBG > 140 mg/dL -> " +
            (0.5 * currRate < 0.5
              ? "inform responsible doctor"
              : "restart at rate = " +
                (roundHalf(0.5 * currRate) +
                  " ml/hr ( if CBG start to reincrease, check every 1 hour until > 140)"))
        );
      }
      ...
      if (differenceCBG > 40) {
        ...
        if (y <= 20) {
          ...
          setResultsElement(
            "New rate is greater than 20 ml/hr, please contact relevant physician"
          );
        }
        ...
        setResultsElement(
          "DC insulin, repeat CBG every 30 minutes, when it starts to reincrease and CBG >= 140 " +
            (y < 0.5
              ? "Inform responsible doctor"
              : "restart at rate = " + y + " ml/hr")
        );
      }
      ...
      if (differenceCBG > 60) {
        ...
        y = Number(currRate) - 2 * delta;
        setResultsElement(
          "DC insulin, repeat CBG every 60 minutes, when it starts to reincrease and CBG >= 140 " +
            (y < 0.5
              ? "Inform responsible doctor"
              : "restart at rate = " + y + " ml/hr")
        );
    ...
    if (currCBG > 300) {
      setResultsElement2("Check Acetone in Urine");
    }
  };
